...
class FaceNetService {
  ...
  dynamic predict() {
    ...
    return searchEmployee(this._predictedData);
  }
  ...
  dynamic searchEmployee(List predictedData) {
    Globals.setAttendanceId("null");
    dynamic predRes;
    return employeeOperations.getAllEmployees().then((result) {
      if (result.length > 0) {
        double minDist = 999;
        double currDist = 0.0;
        ...
        for (int i = 0; i < result.length; i++) {
          if (result[i]['imageData'].toString() != "") {
            ...
            if (currDist <= threshold && currDist < minDist) {
              minDist = currDist;
              predRes = result[i]['employeeId'].toString();
            } else {
              print("Facial Recognition Error");
            }
            Globals.setAttendanceId(predRes.toString());
          }
        }
      }
    });
